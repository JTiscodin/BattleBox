...
public class GameTestCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleJoinGame(Player player, String[] args) {
        ...
        if (game.addPlayer(player)) {
            gameManager.addPlayerToGame(player, gameName);
    ...
    private void handleShowTeams(Player player, String[] args) {
        ...
        if (gameName != null) {
            ...
            if (game == null) {
                player.sendMessage(ChatColor.RED + "You're not in any game! Specify game name: /gametest teams <game-name>");
                return;
            }
        }
    ...
    private void handleCheckWinner(Player player, String[] args) {
        ...
        if (gameName != null) {
            ...
            game = gameManager.getPlayerGame(player);
            if (game == null) {
                player.sendMessage(ChatColor.RED + "You're not in any game! Specify game name: /gametest winner <game-name>");
                return;
            }
        }
    ...
    private void handleWoolCount(Player player, String[] args) {
        ...
        if (gameName != null) {
            ...
            game = gameManager.getPlayerGame(player);
            if (game == null) {
                player.sendMessage(ChatColor.RED + "You're not in any game! Specify game name: /gametest woolcount <game-name>");
                return;
            }
        }
    ...
    private void handleGameInfo(Player player, String[] args) {
        ...
        for (Game.TeamColor color : Game.TeamColor.values()) {
            player.sendMessage(ChatColor.WHITE + "  " + color.chatColor + color.displayName + ": " + teamCounts.get(color) + " players");
        }
    }
    ...
    private void sendHelpMessage(Player player) {
        ...
        player.sendMessage(ChatColor.AQUA + "/gametest create <game> <arena>" + ChatColor.WHITE + " - Create a test game");
        ...
        player.sendMessage(ChatColor.AQUA + "/gametest woolcount [game]" + ChatColor.WHITE + " - Show wool count in arena box");
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            List<String> subCommands = Arrays.asList("create", "join", "leave", "teams", "winner", "woolcount", "list", "info", "delete");
            ...
            if (subCommand.equals("join") || subCommand.equals("teams") || subCommand.equals("winner") || 
                ...
                // Tab complete with game names (we'd need to add a method to get all game names)
